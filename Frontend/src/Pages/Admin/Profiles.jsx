...
const AdminProfiles = () => {
  ...
  const [confirmTarget, setConfirmTarget] = useState(null); // { id, is_active, owner }
  ...
  const handleConfirmToggle = async () => {
    const { id, is_active, owner } = confirmTarget;
    ...
    try {
      ...
      if (!res.ok) throw new Error(result.message || "Failed to update profile status");

      setProfiles(prev => prev.map(p =>
        p.id === id ? { ...p, is_active: nextActive, status: nextActive ? "Active" : "Inactive" } : p
      ));
      setNotice(`${owner}'s profile ${nextActive ? "activated" : "deactivated"} successfully.`);
    } catch (err) {
  ...
  return (
    <div className="admin-page">
                          ...
                          <td>
                            <div className="admin-row-actions">
                              ...
                              <button
                                ...
                                onClick={() => setConfirmTarget({ id: profile.id, is_active: profile.is_active, owner: profile.owner })}
                              >
      ...
      <ConfirmModal
        ...
        title={confirmTarget?.is_active ? "Deactivate Profile" : "Activate Profile"}
        message={confirmTarget?.is_active
          ? `Deactivate ${confirmTarget?.owner}'s profile? Their public profile will be hidden.`
          : `Activate ${confirmTarget?.owner}'s profile? Their public profile will be visible again.`}
